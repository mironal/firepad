...
firepad.FirebaseAdapter = (function(global) {
  ...
  function FirebaseAdapter(ref, userId, userColor, encrypter, decrypter) {
    this.ref_ = ref;
    this.ready_ = false;
    ...
    function doTransaction(revisionId, revisionData) {
      self.ref_
        ...
        .transaction(
          function(current) {
            if (current === null) {
              if (typeof this.encrypter === "function") {
                util.log("doTransaction: enc");
                const enc = this.encrypter(revisionData);
                if (enc) {
                  return this.encrypter(revisionData);
                }
              }
              return revisionData;
            }
  ...
  FirebaseAdapter.prototype.monitorHistory_ = function() {
    ...
    this.ref_.child("checkpoint").once("value", function(s) {
      ...
      var checkpoint = s.val() || {};
      if (typeof this.decrypter === "function") {
        var dec = this.decrypter(checkpoint);
        if (dec) {
          checkpoint = dec;
        }
      }

      var revisionId = checkpoint.id;
      var op = checkpoint.o;
      var author = checkpoint.a;
  ...
  FirebaseAdapter.prototype.monitorHistoryStartingAt_ = function(revision) {
    ...
    setTimeout(function() {
      self.firebaseOn_(historyRef, "child_added", function(revisionSnapshot) {
        ...
        var value = revisionSnapshot.val();
        if (value && typeof this.decrypter === "function") {
          var dec = this.decrypter(value);
          if (dec) {
            value = dec;
          }
        }

        self.pendingReceivedRevisions_[revisionId] = value;
        if (self.ready_) {
  ...
  FirebaseAdapter.prototype.saveCheckpoint_ = function() {
    ...
    if (typeof this.encrypter === "function") {
      const enc = this.encrypter(value);
      if (enc) {
        value = enc;
      }
    }

    this.ref_.child("checkpoint").set(value);
  };
