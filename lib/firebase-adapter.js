...
firepad.FirebaseAdapter = (function(global) {
    ...
    function doTransaction(revisionId, revisionData) {
      self.ref_
        ...
        .transaction(
          function(current) {
            if (current === null) {
              if (typeof self.encrypter === "function") {
                const enc = self.encrypter(revisionData);
                if (enc) {
                  return enc
                }
              }
  ...
  FirebaseAdapter.prototype.monitorHistory_ = function() {
    ...
    this.ref_.child("checkpoint").once("value", function(s) {
      ...
      if (typeof self.decrypter === "function") {
        var dec = self.decrypter(checkpoint);
        if (dec) {
          checkpoint = dec;
        }
      }
  ...
  FirebaseAdapter.prototype.monitorHistoryStartingAt_ = function(revision) {
    ...
    setTimeout(function() {
      self.firebaseOn_(historyRef, "child_added", function(revisionSnapshot) {
        ...
        if (value && typeof self.decrypter === "function") {
          var dec = self.decrypter(value);
          if (dec) {
            value = dec;
          }
        }
